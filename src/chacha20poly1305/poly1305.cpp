...
poly1305_donna_mul:
	...
	if (!inlen && buf == m_vec.end()) goto poly1305_donna_finish;
	/* final bytes in the current buffer, and maybe more from the next */
poly1305_donna_atmost15bytes:
	...
	if (!j && buf == m_vec.end()) goto poly1305_donna_finish;
	j++;
	if (j <= 16)
	{
		j--;
	...
	if (j == 17) goto poly1305_donna_bridge_buffer;
